...
class BaseModel(nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        total_gflops = 0.0  # 新增：总GFLOPs累加器
        for m in self.model:
            if m.f != -1:
                x = y[m.f] if isinstance(m.f, int) else [x if j == -1 else y[j] for j in m.f]
            if profile:
                layer_gflops = self._profile_one_layer(m, x, dt)  # 获取单层GFLOPs
                total_gflops += layer_gflops  # 累加
            x = m(x)
        ...
        if profile:  # 新增：输出总GFLOPs
            LOGGER.info(f"{colorstr('Total GFLOPs:')} {total_gflops:.2f}")
        return x
    ...
    def _profile_one_layer(self, m, x, dt):
        ...
        flops = thop.profile(m, inputs=[x.copy() if c else x], verbose=False)[0] / 1e9 * 2 if thop else 0  # 计算FLOPs
        t = time_sync()
        for _ in range(10):
        ...
        return flops  # 关键修改：返回该层的GFLOPs值
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m is WaveletConv:
                c2 = args[0]  # 输出通道数
                wave = args[1] if len(args) > 1 else 'haar'  # wavelet 类型
                args = [c1, c2, wave]  # 明确指定参数，避免多余参数
            else:
                if c2 != nc:  # 如果 c2 不等于类别数（例如 Classify 输出）
